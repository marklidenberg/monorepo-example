...
class Installer:
    ...
    def _which_python(self):
        ...
        if fallback is None:
            raise RuntimeError("No python executable found in shell environment. Tried: " + str(allowed_executables))
    ...
    def display_pre_message(self):
        ...
        if not self._modify_path:
            ...
            if "fish" in SHELL:
                ...
                profiles = [
                    colorize("comment", p.replace(os.getenv("HOME", ""), "$HOME")) for p in self.get_unix_profiles()
                ]
    ...
    def display_post_message(self, version):
        ...
        if WINDOWS:
            ...
            poetry_home_bin = POETRY_BIN.replace(os.getenv("HOME", ""), "$HOME")
            kwargs["poetry_home_env"] = colorize("comment", POETRY_ENV.replace(os.getenv("HOME", ""), "$HOME"))
...
def main():
    ...
    parser.add_argument(
        ...
        dest="file",
        action="store",
        help=("Install from a local file instead of fetching the latest version " "of Poetry available online."),
    )
